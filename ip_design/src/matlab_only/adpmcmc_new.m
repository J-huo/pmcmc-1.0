...
if (problem==0)
    ...
    cov = [fix_cov_std 0; 0 fix_cov_std/10];%diag(repmat(fix_cov_std,theta_dim,1));      %fixed cov-matrix
...
elseif (problem==1) 
    ...
    %genome
    thetas = nan(theta_dim,iterations);
    liks = zeros(1,iterations);
    particles = zeros(state_dim,state_count*iterations);
    prop_thetas = zeros(theta_dim,iterations);
    prop_liks = zeros(1,iterations);
    urnd_matlab = zeros(1,iterations);
    ...
    cov = diag(repmat(fix_cov_std,theta_dim,1));      %fixed cov-matrix
