...
Xint32 *inputs_in_ptr_ddr = (Xint32 *) inputs_IN_DEFINED_MEM_ADDRESS;
Xint32 *inputs_in_ptr_ddr_1 = (Xint32 *) inputs_IN_DEFINED_MEM_ADDRESS_1;
Xint32 *inputs_in_ptr_ddr_2 = (Xint32 *) inputs_IN_DEFINED_MEM_ADDRESS_2;
Xint32 *inputs_in_ptr_ddr_3 = (Xint32 *) inputs_IN_DEFINED_MEM_ADDRESS_3;
Xint32 *outputs_out_ptr_ddr = (Xint32 *) outputs_OUT_DEFINED_MEM_ADDRESS;
			...
			if (packet_type==1) //reset IP
						{
					...
					else if (packet_type==2) //start IP
							{
								...
								XFoo_SetByte_inputs_in_offset(&xcore,inputs_IN_DEFINED_MEM_ADDRESS);
								XFoo_SetByte_outputs_out_offset(&xcore,outputs_OUT_DEFINED_MEM_ADDRESS);
					...
					else if (packet_type==3) //write data DDR
					{
						...
						//{
							...
							//case 0: //inputs_in
							if (DEBUG)
								printf("write inputs_in\n\r");
							memcpy(inputs_in_ptr_ddr + packet_internal_ID*30000 + (ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
							//break;
							/*
							case 1: //inputs_in
							if (DEBUG)
								printf("write inputs_in\n\r");
							memcpy(inputs_in_ptr_ddr_1+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
							break;

							case 2: //inputs_in
							if (DEBUG)
								printf("write inputs_in\n\r");
							memcpy(inputs_in_ptr_ddr_2+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
							break;

							case 3: //inputs_in
							if (DEBUG)
								printf("write inputs_in\n\r");
							memcpy(inputs_in_ptr_ddr_3+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
							break;
							 */
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							...
							case 0: //outputs_in
							if (DEBUG)
								printf("read outputs_out\n\r");
							memcpy(outvec,outputs_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							break;
						...
						if (FLOAT_FIX==1)  //fixed-point
						{
							...
							{
								tmp_float=0.0;
								outvec[ETH_PACKET_LENGTH_RECV-2]=*(Xint32*)&tmp_float; //IP is running: 1=YES, 0=NO
								if (flag_t == 0){
									//Stop timer
									CntValue2 = IpTimer(TIMER_DEVICE_ID,1);
									flag_t = 1;
...

{
	...
	if (p->len == (ETH_PACKET_LENGTH)*sizeof(int32_t)){
		...
		else if (packet_type==3) //write data DDR
		{
			...
			if (DEBUG)
				printf("write inputs_in\n\r");
			memcpy(inputs_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
			break;
